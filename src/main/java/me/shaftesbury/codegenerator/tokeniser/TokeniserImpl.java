...
public class TokeniserImpl extends VoidVisitorAdapter<List<IToken>> {
    ...
    @Override
    public void visit(final BlockStmt n, final List<IToken> arg) { // eg { new A().doTheThing(); }
    ...
    @Override
    public void visit(final ExpressionStmt n, final List<IToken> arg) { // eg new A().doTheThing();
        n.getExpression().accept(this, arg);
        arg.add(SEMICOLON);
    ...
    public void visit(final MethodCallExpr n, final List<IToken> arg) {
        n.getArguments().forEach(p -> p.accept(this, arg));
        n.getScope().ifPresent(l -> l.accept(this, arg)); // new A()
        arg.add(DOT);
        final StringBuilder name = new StringBuilder();
        n.getName().accept(nameTokeniserFactory.get(), name); // doTheThing
        arg.addAll(List.of(FunctionName.of(name.toString()), STARTFUNCTIONPARAMETERS, ENDFUNCTIONPARAMETERS));
        n.getTypeArguments().ifPresent(l -> l.forEach(v -> v.accept(this, arg)));
        n.getComment().ifPresent(l -> l.accept(this, arg));
    }

    @Override //  n = @Test void testFunction() { new A().doTheThing(); }
    public void visit(final MethodDeclaration n, final List<IToken> arg) {
        n.getAnnotations().forEach(p -> p.accept(this, arg));
        n.getType().accept(this, arg); // void
        final StringBuilder name = new StringBuilder();
        n.getName().accept(nameTokeniserFactory.get(), name);
        arg.addAll(List.of(FunctionName.of(name.toString()), STARTFUNCTIONPARAMETERS));
        n.getParameters().forEach(p -> p.accept(this, arg));
